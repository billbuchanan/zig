...
pub fn main() !void {
    ...
    const Alice_public = try crypto.ecc.Curve25519.basePoint.mul(Alice_private);

    var r: [32]u8 = undefined;
    crypto.random.bytes(&r);

    const R = try crypto.ecc.Curve25519.basePoint.mul(r);

    const S = try crypto.ecc.Curve25519.mul(Alice_public, r);

    // Derive a 32-bit key with HKDF-Sha256
    var derived_key: [32]u8 = undefined;

    var prk = crypto.kdf.hkdf.HkdfSha256.extract("Salty", S.toBytes()[0..32]);
    ...
    const K = try crypto.ecc.Curve25519.mul(R, Alice_private);

    var derived_key2: [32]u8 = undefined;
    prk = crypto.kdf.hkdf.HkdfSha256.extract("Salty", K.toBytes()[0..32]);
    ...
    try stdout.print("ECIES with Curve 25519\n", .{});

    try stdout.print("\nMessage:\t{s}\n", .{message});

    try stdout.print("\nAlice secret key:\t{x}\n", .{Alice_private});
    try stdout.print("Alice public key:\t{x}\n", .{Alice_public.toBytes()});
