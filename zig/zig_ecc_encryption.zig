...
pub fn encrypt(data: []const u8, publicKey: []const u8) ![]u8 {
    const secret = try crypto.ecc.P256.generateKeyPair();
    const sharedSecret = try crypto.deriveSharedSecret(secret.privateKey, publicKey);
    const cipherText = try crypto.encrypt(data, sharedSecret);
    return cipherText;
}
