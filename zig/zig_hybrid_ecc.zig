...
pub fn main() !void {
    ...
    const Alice_public = try crypto.ecc.Secp256k1.basePoint.mul(Alice_private, std.builtin.Endian.big);

    var r: [32]u8 = undefined;
    crypto.random.bytes(&r);

    const R = try crypto.ecc.Secp256k1.basePoint.mul(r, std.builtin.Endian.big);

    const S = try crypto.ecc.Secp256k1.mul(Alice_public, r, std.builtin.Endian.big);
    ...
    const K = try crypto.ecc.Secp256k1.mul(R, Alice_private, std.builtin.Endian.big);
    ...
    try stdout.print("ECIES with secp256k1\n", .{});
