...
pub fn main() !void {
    ...
    const kp = crypto.kem.ml_kem.MLKem512.KeyPair.generate();

    const ct = crypto.kem.ml_kem.MLKem512.PublicKey.encaps(kp.public_key, seed);
    ...
    const sharedkey = try crypto.kem.ml_kem.MLKem512.SecretKey.decaps(kp.secret_key, &ct.ciphertext);
    ...
    try stdout.print("Hybrid ML-KEM-512 Encryption\n", .{});
