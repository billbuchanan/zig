...
pub fn main() !void {
    ...
    var key: [aead.aes_gcm.Aes128Gcm.key_length]u8 = undefined;
    std.crypto.random.bytes(&key);
    var nonce: [aead.aes_gcm.Aes128Gcm.nonce_length]u8 = undefined;
    ...
    aead.aes_gcm.Aes128Gcm.encrypt(ciphertext, &tag, m, ad, nonce, key);

    const m2 = try allocator.alloc(u8, m.len);
    defer allocator.free(m2);

    const ct = ciphertext;
    try aead.aes_gcm.Aes128Gcm.decrypt(m2, ct, tag, ad, nonce, key);

    try stdout.print("AES-GCM (128-bit)\n", .{});
    try stdout.print("\nMessage: {s}\n", .{m});
    try stdout.print("\nKey:\t{x} \n", .{key});
