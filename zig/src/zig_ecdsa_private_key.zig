...
pub fn main() !void {
    ...
    // Crack key with a leak of r
    // priv=r−1(s⋅k−H(M))
    const r_inv = r.invert();

    const pr = r_inv.mul(s.mul(k).sub(z));

    try stdout.print("\n\nPrivate key recovered is {x}\n", .{pr.toBytes(std.builtin.Endian.big)});
    if (pr.equivalent(d)) {
        try stdout.print("\nThe private key has been recovered!", .{});
    } else {
        try stdout.print("\nThe private key has NOT been recovered!", .{});
    }
    try stdout.flush();
}
