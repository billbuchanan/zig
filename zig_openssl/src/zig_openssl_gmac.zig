...
pub fn main() !void {
    ...
    //   const cipher: []const u8 = "AES-128-GCM";
    const cipher = args[2];

    var out_len: usize = 0;

    const mac = c.EVP_MAC_fetch(null, "GMAC", null) orelse return error.MacFetchFailed;
    defer c.EVP_MAC_free(mac);

    const ctx = c.EVP_MAC_CTX_new(mac) orelse return error.MacCtxNewFailed;
    defer c.EVP_MAC_CTX_free(ctx);

    const iv = try allocator.alloc(u8, 12); // 12 bytes
    ...
    params[1] = c.OSSL_PARAM_construct_octet_string(c.OSSL_CIPHER_PARAM_IV, @ptrCast(@constCast(iv)), iv.len);
