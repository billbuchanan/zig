...
pub fn main() !void {
    ...
    try wcCheck(c.wc_ShaUpdate(&sha1, msg.ptr, @intCast(msg.len)), "wc_ShaUpdate");
    ...
    try wcCheck(c.wc_Sha256Update(&sha256, msg.ptr, @intCast(msg.len)), "wc_Sha256Update");
    ...
    //   try wcCheck(c.wc_Sha512Update(&sha512, msg.ptr, @intCast(msg.len)), "wc_Sha512Update");
    ...
    try stdout.print("SHA-1 {x}", .{sha1_out[0..]});
    try stdout.print("SHA-256  {x}", .{sha256_out[0..]});
    //  try stdout.print("SHA-512  {x}", .{sha512_out[0..]});
    //  try stdout.print("SHA3-256  {x}", .{sha3_256_out[0..]});
    //  try stdout.print("SHA3-512 {x}", .{sha3_512_out[0..]});
    // try stdout.print("BLAKE2b-512  {x}", .{b2b_out[0..]});
